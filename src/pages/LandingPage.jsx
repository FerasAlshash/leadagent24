...
export default function LandingPage({ onGetStarted, onSignIn, onNavigate }) {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans selection:bg-emerald-600 selection:text-white flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-2xs">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3.5">
            <LeadAgentLogo className="w-10 h-10 shadow-sm" />
            <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white transition-all shadow-sm cursor-pointer"
            >
      ...
      <section className="relative pt-16 pb-20 sm:pt-24 sm:pb-28 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-semibold mb-6 shadow-2xs">
            ...
            <span>24/7 Autonomous B2B Prospecting & Outbound Engine</span>
          </div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            <button
              type="button"
              onClick={onGetStarted}
              className="w-full sm:w-auto flex items-center justify-center gap-2.5 px-8 py-3.5 rounded-xl text-sm font-bold bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white shadow-sm transition-all cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={onSignIn}
              className="w-full sm:w-auto px-6 py-3.5 rounded-xl text-sm font-semibold bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-all cursor-pointer"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((f, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className="p-6 rounded-3xl bg-[#f8fafc] border border-slate-200 hover:border-emerald-300 hover:shadow-md transition-all"
                >
                  <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-emerald-600 mb-4 shadow-2xs">
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {workflowSteps.map((ws) => (
              <div 
                key={ws.step}
                className="p-7 rounded-3xl bg-white border border-slate-200 flex flex-col justify-between shadow-2xs hover:border-emerald-300 transition-all"
              >
                <div>
      ...
      <section id="providers" className="py-16 sm:py-20 bg-white border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-6 sm:px-10">
          <div className="text-center max-w-2xl mx-auto mb-14">
            <span className="text-xs font-bold uppercase tracking-wider text-emerald-700">
              Zero Vendor Lock-In
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {providers.map((p, idx) => (
              <div 
                key={idx}
                className="p-6 rounded-3xl bg-[#f8fafc] border border-slate-200 hover:border-emerald-300 hover:shadow-md transition-all flex flex-col justify-between"
              >
                <div>
                  ...
                  <span className="text-[11px] text-emerald-700 font-semibold block mb-2">{p.type}</span>
      ...
      <section id="security" className="py-16 sm:py-20 bg-[#f8fafc]">
        <div className="max-w-7xl mx-auto px-6 sm:px-10">
          <div className="p-8 sm:p-12 rounded-3xl bg-white border border-slate-200 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            <div className="shrink-0 flex flex-col gap-3 w-full sm:w-auto">
              <button
                type="button"
                onClick={onGetStarted}
                className="px-8 py-3.5 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white transition-all shadow-sm cursor-pointer text-center"
              >
              ...
              <button
                type="button"
                onClick={() => onNavigate ? onNavigate('/about') : null}
                className="px-8 py-3.5 rounded-xl text-xs font-semibold bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 transition-all cursor-pointer text-center"
              >
